...
def  main(dataset,size=400,alpha=0.0275,window=3,negSampling=3,threshold=-1,eta=1,d=0,W=[],Importance=10,lamba=1.0,g=0):
    ...
    outVector = np.random.uniform(low=-1,high=1,size=(len(vocabulary),size))
...
def LearningEmbedding(alpha,window,negSampling,size,eta,d,W,Importance,init,end,inVector,outVector,train,lamba=1.0):
    ...
    for c in range(init,end):
        ...
        mvec = np.mean(outVector.array[contexts],axis=0)
        ...
        outVector.array[contexts] += Dmvec
